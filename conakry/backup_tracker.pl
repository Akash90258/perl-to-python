...
my $opco_name = 'conakry'; chomp($opco_name);
...
my $to = 'gnoc.1st.la.mtn.rsaa@ericsson.com,PDLFOINMTN@pdl.internal.ericsson.com,PDLHIFTMAN@pdl.internal.ericsson.com';
...
sub read_inp{
	...
	my $array_ref_geo = [];
	if ($user_l =~ /sdp/ig || $user_l =~ /vs/ig) {
		$array_ref_geo = $users_details->{"$user-geo-red"};
		$inp_dir_hash->{"$user-geo-red"}->{$inp_dir} = 1;
	}
	...
	if ($inp_size == 0) {
		...
		foreach (@$array_ref_geo) {
			$parsed_hash->{"$user-geo-red"}->{$host}->{$ip}->{$_} = $issue1;
		}
	}
	else 
	{
		...
		{	
			...
			foreach (@$array_ref_geo) {
				$parsed_hash->{"$user-geo-red"}->{$host}->{$ip}->{$_} = 'N/A';
			}
			foreach my $inp_name (@inp_files) {
				...
				if ($user_l eq 'sdp') {	
					...
					if ($inp_name =~ /TTMonitorStandby.inp/ig && $parsed_hash->{"$user-geo-red"}->{$host}->{$ip}->{'geo-redundancy.inp'} eq 'N/A')
					{
			...
			if ($sdp_geo_check == 2 && $parsed_hash->{"$user-geo-red"}->{$host}->{$ip}->{'geo-redundancy.inp'} eq 'N/A') {
				$parsed_hash->{"$user-geo-red"}->{$host}->{$ip}->{'geo-redundancy.inp'} = 'Success';
			}			
		}
		else {
...
sub dbn{
	...
	elsif ($data =~ /rman\_$date2.*?Recovery\s+Manager\s+complete/ig) {
		$status = 'Success';
	} 
