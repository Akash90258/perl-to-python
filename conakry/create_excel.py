...
try:
    ...
    for key, values in parsed_hash.items():
        ...
        if 'SDP' == opco.upper():
            ...
            for row in final_data:
                ...
                template_df_map[sheet_name] = template_df_map[sheet_name] \
            ...
            # -------------For SDP Geo Redundancy
            final_data = nodes_iterator_sdp_geo(
                parsed_hash['sdp-geo-red'], updated_pma_dict[opco.lower()]
            )
            sheet_name = "SDP Geo Redundancy Sheet"
            for row in final_data:
                template_df_map[sheet_name] = template_df_map[sheet_name] \
                    .append(row, ignore_index=True)
            template_df_map[sheet_name].rename(
                columns={'Todays_date': datetime.today().strftime("%d-%b-%Y")},
                inplace=True
            )
        ...
        if 'MINSAT' == opco.upper():
            ...
            # ------------------
        if 'CCN' == opco.upper():
            sheet_name = "CCN_DBN backup_Daily"
        ...
        # ------------------
        if 'EMA' == opco.upper():
            sheet_name = "EMA proclogs_Backup _Daily"
        ...
        # ------------------
        if 'CRS' == opco.upper():
            sheet_name = "CRS DB backup_Daily"
    ...
    # print("===================================")
    # print(template_df_map["VS_Daily"])
    # print("===================================")
    writer.save()
