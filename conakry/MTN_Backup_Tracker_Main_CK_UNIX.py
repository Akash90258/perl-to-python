...
Config_File = glob.glob("/home/conakry/PM-Automation/MTN/CONFIG/CONFIG_PYTHON_UNIX.xlsx")
...
def Write_Normal_Output(df_Config,template_to_df_map,opco):
    ...
    for i,row_output_page in df_Config.loc[df_Config['CODENAME']=='ENTITY_OUTPUT_PAGE',['CODEVALUE','CODEVALUE1','CODEVALUE2']].iterrows():
        ...
        if opco == 'CK' and row_output_page['CODEVALUE'] == 'SDP-GEO-RED':
            template_to_df_map[row_output_page['CODEVALUE']] = template_to_df_map[row_output_page['CODEVALUE']].drop(columns="Node IP")
            template_to_df_map[row_output_page['CODEVALUE']] = template_to_df_map[row_output_page['CODEVALUE']].rename(columns={"Status": ""+datetime.datetime.today().strftime("%d-%b-%Y")})
            template_to_df_map[row_output_page['CODEVALUE']].to_excel(writer, row_output_page['CODEVALUE1'],
                                                                      index=False,
                                                                      header=True)

        elif row_output_page['CODEVALUE'] not in ['Summary','Dashboard_Backup_Success_Fail','Dashboard_Performance']:
...
def Send_Email_SMTP(Attachment_Path,Attachment_Name,opco,df_Config):
	...
	toaddr = ['gnoc.1st.la.mtn.rsaa@ericsson.com','PDLMTNFMIN@pdl.internal.ericsson.com','PDLFOINMTN@pdl.internal.ericsson.com','PDLHIFTMAN@pdl.internal.ericsson.com','aditya.k.kumar@ericsson.com']#df_Config[df_Config['CODENAME']=='EMAIL_TO_SEND']['CODEVALUE'].iloc[0]
	COMMASPACE = ', '
