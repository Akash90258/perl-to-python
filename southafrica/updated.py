...
todays_datetime = datetime.datetime.today() - datetime.timedelta(1)
...
def tape(data, date1, date2, inp_dir_path):
    ...
    status = ""
    fail_reason = ""
    regex1 = '(cfbackup.*?{}\W+No\s+such\s+file\s+or\s+directory)'.format(curr_date2)
    regex2 = '(cfbackup.*?{})'.format(curr_date2)
    regex3 = '(INFO:root.*?Filesystem.*backup started.*?at {})'.format(curr_date)
    regex4 = '(INFO:root.*?Filesystem.*backup ended.*?at {})'.format(curr_date)
    if len(re.findall(regex3, data)) > 0:
        status = 'Success'
    elif len(re.findall(regex1, data)) > 0 or len(re.findall(regex2, data)) > 0:
        status = 'Success'
    elif len(re.findall(regex4, data)) > 0:
        status = 'Success'
    else:
        status = 'Fail'
