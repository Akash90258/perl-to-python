...
my $opco_name = 'southafrica'; chomp($opco_name);
...
my $month_date = `date '+%b %d'`; chomp($month_date);
...
my $pre_date5 = `date '+%y%m%d' -d '-7 day'`; chomp($pre_date5);
...
my $to = 'gnoc.1st.la.mtn.rsaa@ericsson.com,PDLFOINMTN@pdl.internal.ericsson.com,PDLHIFTMAN@pdl.internal.ericsson.com,prachi.p@ericsson.com';
my $cc = 'mohd.ezaz@ericsson.com';

#my $to = 'PDLGNOCBOM@pdl.internal.ericsson.com,PDLGNOCBOM@pdl.internal.ericsson.com,PDLGNOCBOM@pdl.internal.ericsson.com';
#my $cc = '';
		...
		{	
			...
			foreach my $inp_name (@inp_files) {
				...
				if ($user_l eq 'air') {
					...
					if ($inp_name =~ /bura_backup.inp/ig || $inp_name =~ /nfs.inp/ig) {
				...
				if ($user_l eq 'ece') {
                                        my $status;
                                        if ($inp_name =~ /dbbk.inp/ig) {
                                                $status = db_bkup($file_data);
                                                $parsed_hash->{$user}->{$host}->{$ip}->{$inp_name} = $status;
                                        }
                                }

				##----------------- SAPC fs_backup Check ----------##
				 if ($user_l eq 'sapc') {
                                        my $status;
                                        if ($inp_name =~ /fsb.inp/ig) {
                                                $status = fs_backup($file_data);
                                                $parsed_hash->{$user}->{$host}->{$ip}->{$inp_name} = $status;
                                        }
                                }
				...
				if ($user_l eq 'occ') {
					...
					if ($inp_name =~ /backup.inp/ig) {
						($status, $fail_reason) = tape($file_data, $curr_date, $curr_date5, $pre_date5, $inp_dir_path);
						$parsed_hash->{$user}->{$host}->{$ip}->{$inp_name} = "$status^^$fail_reason";
					}
				}	
				...
				if ($user_l eq 'sdp') {	
					my $status; my $fail_reason;
					if ($inp_name =~ /TTMonitor.inp/ig || $inp_name =~ /TTMonitorlog.inp/ig) {
						$sdp_geo_check++;
						my @geo_str = (split("\n", $file_data));
						my $arr_len = @geo_str;
						if ($arr_len > 4) {
					...
					if ($inp_name =~ /bura_backup.inp/ig) {
						#print $file_data; die;
						($status, $fail_reason) = tape($file_data, $inp_dir_path);
						$parsed_hash->{$user}->{$host}->{$ip}->{$inp_name} = "$status^^$fail_reason";
					}
				}		
				...
				if ($user_l eq 'vs') {
					my $status;
					if ($inp_name =~ /ora.inp/ig || $inp_name =~ /ora_archive.inp/ig) {
				...
				##----------------- EMA proclog and sogconfig,software and zookeep up Check -----------------##

				if ($user_l eq 'ema') {
					my $status;
					if ($inp_name =~ /LDE_backup_NLPRGW.inp/ig) {
						$status = proclog($file_data, $month_date, $curr_date2);
						$parsed_hash->{$user}->{$host}->{$ip}->{$inp_name} = $status;
					}
					if ($inp_name =~ /LDE_backup.inp/ig) {
						$status = sogconfig($file_data, $curr_date4);
						$parsed_hash->{$user}->{$host}->{$ip}->{$inp_name} = $status;
						}
						
					if ($inp_name =~ /software.inp/ig) {
                                                $status = software($file_data, $month_date, $curr_date2);
						my ($pre_status, $pre_bkp_date) = manage_weekly_bkp($status);
						$status = "$pre_status [$pre_bkp_date]";
                                                $parsed_hash->{$user}->{$host}->{$ip}->{$inp_name} = $status;
                                        }
                                        if ($inp_name =~ /zookeeper.inp/ig) {
                                                $status = zookeeper($file_data, $curr_date4);
						my ($pre_status, $pre_bkp_date) = manage_weekly_bkp($status);
                                                $status = "$pre_status [$pre_bkp_date]";
                                                $parsed_hash->{$user}->{$host}->{$ip}->{$inp_name} = $status;				
					}
...
sub tape{
	...
	my $status = ''; my $fail_reason = '';
	my $inp_dir_path = shift;
	if ($data =~ /INFO.*?root.*?Filesystem.*?backup\s+started\s+at\s+$curr_date/igs) {
		$status = 'Success';
	}
	elsif ($data !~ /cfbackup.*?$curr_date2\W+No\s+such\s+file\s+or\s+directory/igs && $data =~ /cfbackup.*?$curr_date2/igs) {
		$status = 'Success';
	}
	elsif ($data =~ /INFO.*?root.*?Filesystem.*?backup\s+ended\s+at\s+$curr_date/igs) {
		$status = 'Success';
	}
	else {
		$status = 'Fail';
...
sub db_bkup{
	...
	my $status = '';
        my @bkp = (split("\n", $data));
        my $bkpf = $bkp[2];
	 if ($bkpf =~ /Successful/igs) {
                $status = 'Success';
                }
        elsif ($bkpf =~ /Nobkp/igs) {
                $status = '   ';
                }
        else {
                $status = 'Fail';
...
sub filesystem{
	...
	my $date = shift;
	my $status = '';
	if ($data =~ /$date.*?Backup\s+completed/ig) {
		$status = 'Success';
	} else {
		$status = 'Fail';
	}
...
sub proclog{
	...
	my $date = shift;
	my $status = '';
	if ($data =~ /sogadm\s+sog.*?$month\s+$day1.*?$date/ig || $data =~ /sogadm\s+sog.*?$month\s+$day2.*?$date/ig) {
		$status = 'Success';
	} else {
		$status = 'Fail';
	}
...
sub sogconfig{
	...
	my $date = shift;
	my $status = '';
	if ($data =~ /sogconfig\_backup.$date/ig) {
		$status = 'Success';
	} else {
		$status = 'Fail';
	}
...
sub software{
        ...
        my $date = shift;
        my $status = '';
        if ($data =~ /config\-bootloaderBackup\-$curr_date2/ig) {
                $status = 'Success';
        } else {
                $status = 'Fail';
        }
...
sub zookeeper{
        ...
        my $date = shift;
        my $status = '';
        if ($data =~ /config\-zookeeperBackup\-$curr_date2/ig) {
                $status = 'Success';
        } else {
                $status = 'Fail';
        }
		...
		foreach my $host (keys %$host_hash) {
			...
			foreach my $ip (keys %$ip_hash) {
				...
				foreach my $inp_name (keys %$inp_hash) {	
					...
					$status = 'Done' if ($status =~ /Success/ig && $status !~ /\d+/ig);
					...
					$worksheet->write($row_num, $fail_flag, $fail_reason, $format6);
				}
			}
		}
...
sub manage_weekly_bkp{
	my $status = shift;
	if ($status eq 'Success') {
		writeFile($path."/ema_soft_zoo.txt", "$status##$curr_date");
	}
	my ($pre_status, $pre_bkp_date) = (split('##', trim(readFile($path."/ema_soft_zoo.txt"))))[0, 1];
	return ($pre_status, $pre_bkp_date);
