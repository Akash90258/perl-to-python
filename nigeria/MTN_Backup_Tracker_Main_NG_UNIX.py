...
Config_File = glob.glob("/home/nigeria/PM-Automation/MTN/CONFIG/CONFIG_PYTHON_UNIX.xlsx")
print('Configuration File:'+Config_File[0])
Configsheet_to_df_map = pd.read_excel(Config_File[0], sheet_name=None)
print('Configuration Loaded..')


def InputFile_Exists_For_Stat(opco, df_Config):
    try:

        Input_File_Path = df_Config[df_Config['CODENAME'] == 'EXTERNAL_FILE_PATH']['CODEVALUE']
        Input_file = glob.glob(df_Config[df_Config['CODENAME'] == 'EXTERNAL_FILE_PATH']['CODEVALUE'].iloc[
                                   0] + "MTN-Nigeria-Backup-Tracker_"+datetime.datetime.today().strftime("%Y-%m")+"*.xlsx")
        todaydate = datetime.date.today()
        if todaydate.day < 15:
            firstday = todaydate.replace(day=1)
            lastday = firstday - datetime.timedelta(1)
            Input_file_past = glob.glob(df_Config[df_Config['CODENAME'] == 'EXTERNAL_FILE_PATH']['CODEVALUE'].iloc[
                                   0] + "MTN-Nigeria-Backup-Tracker_" + lastday.strftime("%Y-%m") + "*.xlsx")
...
def find_date(fileName):
    #return re.search("([0-9]{2}\-[0-9]{2}\-[0-9]{4})", fileName)
...
Input_file = InputFile_Exists_For_Stat(opco,df_Config)
...
df_SDP_Success = df_SDP[df_SDP['TAPE'] == 'Done'][['Date', 'TAPE']].groupby(['Date']).count()
df_SDP_Success = df_SDP_Success.fillna(0)
df_SDP_Failure = df_SDP[df_SDP['TAPE']!='Done'][['Date','TAPE']].groupby(['Date']).count()
#df_SDP_Failure = df_SDP[df_SDP['TAPE'] == 'Not Done'][['Date', 'TAPE']].groupby(['Date']).count()
df_SDP_Failure = df_SDP_Failure.fillna(0)
df_SDP_STAT = pd.concat([df_SDP_Success.rename(index=str, columns={"TAPE": "SUCCESS"}),
                         df_SDP_Failure.rename(index=str, columns={"TAPE": "FAILURE"})], axis=1)
df_SDP_STAT = df_SDP_STAT.fillna(0)
df_SDP_Performance = pd.DataFrame()
df_SDP_Performance['Performance'] = df_SDP_STAT['SUCCESS'] / (df_SDP_STAT['SUCCESS'] + df_SDP_STAT['FAILURE'])


# print(df_SDP_Performance)
print(df_SDP_STAT)
df_OCC_Success = df_OCC[df_OCC['TAPE']=='Done'][['Date','TAPE']].groupby(['Date']).count()
df_OCC_Success = df_OCC_Success.fillna(0)
df_OCC_Failure = df_OCC[df_OCC['TAPE']!='Done'][['Date','TAPE']].groupby(['Date']).count()
#df_OCC_Failure = df_OCC[df_OCC['TAPE']=='Not Done'][['Date','TAPE']].groupby(['Date']).count()
df_OCC_Failure = df_OCC_Failure.fillna(0)
df_OCC_STAT = pd.concat([df_OCC_Success.rename(index =str,columns={"TAPE": "SUCCESS"}),df_OCC_Failure.rename(index =str,columns={"TAPE": "FAILURE"})],axis=1)
df_OCC_STAT = df_OCC_STAT.fillna(0)
df_OCC_Performance = pd.DataFrame()
df_OCC_Performance['Performance'] = df_OCC_STAT['SUCCESS']/(df_OCC_STAT['SUCCESS']+df_OCC_STAT['FAILURE'])
#print(df_OCC_Performance)
print(df_OCC_STAT)


df_NGVS_TAPE_Success = df_NGVS[df_NGVS['TAPE'] == 'Done'][['Date', 'TAPE']].groupby(['Date']).count()
df_NGVS_TAPE_Success = df_NGVS_TAPE_Success.fillna(0)
df_NGVS_TAPE_Failure = df_NGVS[df_NGVS['TAPE'] != 'Done'][['Date', 'TAPE']].groupby(['Date']).count()
df_NGVS_TAPE_Failure = df_NGVS_TAPE_Failure.fillna(0)
df_NGVS_CASSENDRA_Success = df_NGVS[df_NGVS['CASSENDRA'] == 'Done'][['Date', 'CASSENDRA']].groupby(['Date']).count()
df_NGVS_CASSENDRA_Success = df_NGVS_CASSENDRA_Success.fillna(0)
df_NGVS_CASSENDRA_Failure = df_NGVS[df_NGVS['CASSENDRA'] != 'Done'][['Date', 'CASSENDRA']].groupby(['Date']).count()
df_NGVS_CASSENDRA_Failure = df_NGVS_CASSENDRA_Failure.fillna(0)
#print(df_NGVS_CASSENDRA_Success)

df_NGVS_TAPE_STAT = pd.concat([df_NGVS_TAPE_Success.rename(index=str, columns={"TAPE": "SUCCESS"}),
                               df_NGVS_TAPE_Failure.rename(index=str, columns={"TAPE": "FAILURE"})], axis=1)
df_NGVS_TAPE_STAT = df_NGVS_TAPE_STAT.fillna(0)

df_NGVS_CASSENDRA_STAT = pd.concat([df_NGVS_CASSENDRA_Success.rename(index=str, columns={"CASSENDRA": "SUCCESS"}),
                                    df_NGVS_CASSENDRA_Failure.rename(index=str, columns={"CASSENDRA": "FAILURE"})],
                                   axis=1)
df_NGVS_CASSENDRA_STAT = df_NGVS_CASSENDRA_STAT.fillna(0)

print(df_NGVS_TAPE_STAT)
print(df_NGVS_CASSENDRA_STAT)

df_NGVS_Performance = pd.DataFrame()
df_NGVS_Performance['Performance'] = (df_NGVS_TAPE_STAT['SUCCESS'] + df_NGVS_CASSENDRA_STAT['SUCCESS']) / (
            df_NGVS_TAPE_STAT['SUCCESS'] + df_NGVS_TAPE_STAT['FAILURE'] + df_NGVS_CASSENDRA_STAT['SUCCESS'] +
            df_NGVS_CASSENDRA_STAT['FAILURE'])


# print(df_NGVS_Performance)
df_CCN_Success = df_CCN[df_CCN['DBN']=='Done'][['Date','DBN']].groupby(['Date']).count()
df_CCN_Success = df_CCN_Success.fillna(0)
#print(df_CCN_Success)
df_CCN_Failure = df_CCN[df_CCN['DBN']!='Done'][['Date','DBN']].groupby(['Date']).count()
#df_CCN_Failure = df_CCN[df_CCN['DBN']=='Not Done'][['Date','DBN']].groupby(['Date']).count()
df_CCN_Failure = df_CCN_Failure.fillna(0)
#print(df_CCN_Failure)
df_CCN_STAT = pd.concat([df_CCN_Success.rename(index =str,columns={"DBN": "SUCCESS"}),df_CCN_Failure.rename(index =str,columns={"DBN": "FAILURE"})],axis=1)
df_CCN_STAT = df_CCN_STAT.fillna(0)
print(df_CCN_STAT)
df_CCN_Performance = pd.DataFrame()
df_CCN_Performance['Performance'] = df_CCN_STAT['SUCCESS']/(df_CCN_STAT['SUCCESS']+df_CCN_STAT['FAILURE'])
#print(df_CCN_Performance)

df_AIR_Success = df_AIR[df_AIR['TAPE']=='Done'][['Date','TAPE']].groupby(['Date']).count()
df_AIR_Success = df_AIR_Success.fillna(0)
df_AIR_Failure = df_AIR[df_AIR['TAPE']!='Done'][['Date','TAPE']].groupby(['Date']).count()
#df_AIR_Failure = df_AIR[df_AIR['TAPE'] =='Not Done'][['Date','TAPE']].groupby(['Date']).count()
df_AIR_Failure = df_AIR_Failure.fillna(0)
df_AIR_STAT = pd.concat([df_AIR_Success.rename(index =str,columns={"TAPE": "SUCCESS"}),df_AIR_Failure.rename(index =str,columns={"TAPE": "FAILURE"})],axis=1)
df_AIR_STAT = df_AIR_STAT.fillna(0)
df_AIR_Performance = pd.DataFrame()
df_AIR_Performance['Performance'] = df_AIR_STAT['SUCCESS']/(df_AIR_STAT['SUCCESS']+df_AIR_STAT['FAILURE'])
#print(df_AIR_Performance)
print(df_AIR_STAT)

#print(df_SDP_GEO_RED)
df_SDP_GEO_RED_Success = df_SDP_GEO_RED[df_SDP_GEO_RED['GEO-REDUNDANCY']=='Done'][['Date','GEO-REDUNDANCY']].groupby(['Date']).count()
df_SDP_GEO_RED_Failure = df_SDP_GEO_RED[df_SDP_GEO_RED['GEO-REDUNDANCY']!='Done'][['Date','GEO-REDUNDANCY']].groupby(['Date']).count()
#df_SDP_GEO_RED_Failure = df_SDP_GEO_RED[df_SDP_GEO_RED['GEO-REDUNDANCY']=='Not Done'][['Date','GEO-REDUNDANCY']].groupby(['Date']).count()

df_SDP_GEO_RED_STAT = pd.concat([df_SDP_GEO_RED_Success.rename(index =str,columns={"GEO-REDUNDANCY": "SUCCESS"}),df_SDP_GEO_RED_Failure.rename(index =str,columns={"GEO-REDUNDANCY": "FAILURE"})],axis=1)
df_SDP_GEO_RED_STAT.fillna(0)
#df_SDP_GEO_RED_STAT = pd.concat([df_SDP_GEO_RED_Success,df_SDP_GEO_RED_Failure],axis=1)
df_SDP_GEO_RED_Performance = pd.DataFrame()
df_SDP_GEO_RED_Performance['Performance'] = df_SDP_GEO_RED_STAT['SUCCESS']/(df_SDP_GEO_RED_STAT['SUCCESS']+df_SDP_GEO_RED_STAT['FAILURE'])
#df_SDP_GEO_RED_STAT = df_SDP_GEO_RED_STAT.rename(index =str,columns={"GEO-REDUNDANCY": "SUCCESS", "GEO-REDUNDANCY": "Failure"})
#print(df_SDP_GEO_RED_Performance)
#print(df_SDP_GEO_RED_Success)
#print(df_SDP_GEO_RED_Failure)
#print(df_SDP_GEO_RED_STAT)

df_NGVS_GEO_RED_Success = df_NGVS_GEO_RED[df_NGVS_GEO_RED['GEO-REDUNDANCY']=='Done'][['Date','GEO-REDUNDANCY']].groupby(['Date']).count()
df_NGVS_GEO_RED_Success = df_NGVS_GEO_RED_Success.fillna(0)
df_NGVS_GEO_RED_Failure = df_NGVS_GEO_RED[df_NGVS_GEO_RED['GEO-REDUNDANCY']!='Done'][['Date','GEO-REDUNDANCY']].groupby(['Date']).count()
df_NGVS_GEO_RED_Failure = df_NGVS_GEO_RED_Failure.fillna(0)
#print(df_NGVS_GEO_RED_Failure)
df_NGVS_GEO_RED_STAT = pd.concat([df_NGVS_GEO_RED_Success.rename(index =str,columns={"GEO-REDUNDANCY": "SUCCESS"}),df_NGVS_GEO_RED_Failure.rename(index =str,columns={"GEO-REDUNDANCY": "FAILURE"})],axis=1)
df_NGVS_GEO_RED_STAT = df_NGVS_GEO_RED_STAT.fillna(0)
#print(df_NGVS_GEO_RED_Success/(df_NGVS_GEO_RED_Success+df_NGVS_GEO_RED_Failure))
#df_NGVS_GEO_RED_Performance = (df_NGVS_GEO_RED_Success/(df_NGVS_GEO_RED_Success+df_NGVS_GEO_RED_Failure)).rename(index =str,columns={"GEO-REDUNDANCY": "Performance"})
#print(df_SDP_Performance)
#print(df_NGVS_GEO_RED_Failure.empty)
#print(df_NGVS_GEO_RED_Success)
df_NGVS_GEO_RED_Performance = pd.DataFrame()
df_NGVS_GEO_RED_Performance['Performance'] = df_NGVS_GEO_RED_STAT['SUCCESS']/(df_NGVS_GEO_RED_STAT['SUCCESS']+df_NGVS_GEO_RED_STAT['FAILURE'])
#print(df_NGVS_GEO_RED_Performance[['Date','Performance']])
#print(df_NGVS_GEO_RED_STAT)
#print(df_NGVS_GEO_RED_Performance)
#print(df_NGVS_GEO_RED_Success+df_NGVS_GEO_RED_Failure)

df_OVERALL_STAT = df_SDP_STAT+df_CCN_STAT+df_NGVS_TAPE_STAT+df_NGVS_CASSENDRA_STAT+df_AIR_STAT+df_OCC_STAT#+df_SDP_GEO_RED_STAT+df_NGVS_GEO_RED_STAT
print(df_OVERALL_STAT)

df_OVERALL_Performance = pd.DataFrame()
df_OVERALL_Performance['Performance'] = df_OVERALL_STAT['SUCCESS']/(df_OVERALL_STAT['SUCCESS']+df_OVERALL_STAT['FAILURE'])
#print(df_OVERALL_Performance)


#End of Building Statistics
# In[4]:
def Is_Input_File_Copied(opco,df_Config):
    try:
        ...
        #File_name = Input_File_Prefix+yesterday.strftime("%Y-%m-%d")+".xlsx"
            ...
            try:
                if row['Node Type'] not in ['Total','Performance']:
                    if row['Node Type'] == 'SDP': 
                        ...
                        Falure = template_to_df_map[row['Node Type']].loc[template_to_df_map[row['Node Type']]['Node-A']!= 'Done',['Node-A']].count().iloc[0]+template_to_df_map[row['Node Type']].loc[template_to_df_map[row['Node Type']]['Node-B']!= 'Done',['Node-B']].count().iloc[0]#Falure = count - success
...
def Write_Normal_Output(df_Config,template_to_df_map,opco):
    ...
    for i,row_output_page in df_Config.loc[df_Config['CODENAME']=='ENTITY_OUTPUT_PAGE',['CODEVALUE','CODEVALUE1','CODEVALUE2']].iterrows():
        ...
        if row_output_page['CODEVALUE'] not in ['Summary','Dashboard_Backup_Success_Fail','Dashboard_Performance']:
            ...
            if opco == 'NG':
                if row_output_page['CODEVALUE1'] in['SDP_Geo_Redundancy_Status','NGVS_Geo Redundancy_status'] :
                    df_Day_mdy.to_excel(writer, row_output_page['CODEVALUE1'], index=False, startrow=0, startcol=2,header=False)
            ...
            df_OVERALL_STAT.to_excel(writer, row_output_page['CODEVALUE1'], index=True,startrow=1,startcol=0, header=False)
            ...
            df_OVERALL_Performance.to_excel(writer, row_output_page['CODEVALUE1'], index=True, startrow=1,startcol=0, header=False)
            #df_stat['Day'] = pd.Series(pd.to_datetime('today').strftime('%d/%m/%Y'), index=df_stat.index)
            #df_Day.to_excel(writer, row_output_page['CODEVALUE1'], index=False,startrow=int(datetime.datetime.today().strftime("%d")),startcol=0, header=False)
        ...
        elif opco == 'CONG' and row_output_page['CODEVALUE'] =='Summary':
            template_to_df_map['AIR'].to_excel(writer, row_output_page['CODEVALUE'], index=False, startrow=1,startcol=1, header=False)
            template_to_df_map['SDP'].to_excel(writer, row_output_page['CODEVALUE'], index=False, startrow=6,startcol=1, header=False)
            template_to_df_map['CCN'].to_excel(writer, row_output_page['CODEVALUE'], index=False, startrow=11,startcol=1, header=False)
            template_to_df_map['NGVS'].to_excel(writer, row_output_page['CODEVALUE'], index=False, startrow=14,startcol=1, header=False)
            template_to_df_map['CRS'].to_excel(writer, row_output_page['CODEVALUE'], index=False, startrow=21,startcol=1, header=False)
            template_to_df_map['MINSAT'].to_excel(writer, row_output_page['CODEVALUE'], index=False, startrow=24,startcol=1, header=False)
            template_to_df_map['EMA'].to_excel(writer, row_output_page['CODEVALUE'], index=False, startrow=27,startcol=1, header=False)
            template_to_df_map['OCC'].to_excel(writer, row_output_page['CODEVALUE'], index=False, startrow=30,startcol=1, header=False)
            print('Congo summary populated..')
