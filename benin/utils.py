...
def nodes_iterator_air(values, pma_ips):
    ...
    for key, value in values.items():
        ...
        row_data['Status(Backup on Archive Server)'] = status
        final_data.append(row_data)
    return final_data
...
def nodes_iterator_ccn(values, pma_ips):
    ...
    for key, value in values.items():
        ...
        row_data = {}
        if '10.9.0.34' in IP:    
            row_data['IP Address IO1'] = '10.9.0.33'
        elif '10.12.0.73' in IP:    
            row_data['IP Address IO1'] = '10.12.0.33'
        elif '10.9.0.121' in IP:    
            row_data['IP Address IO1'] = '10.9.0.113'
        else:
            row_data['IP Address IO1'] = '10.12.0.113'
        row_data['IP Address IO2'] = IP
        node_name = key
        row_data['Node Name'] = node_name
        status = update_status(list(list(value.values())[0].values())[0])
        row_data['Daily sheduled DBN backup(backup on Disk)'] = status
        final_data.append(row_data)
    return final_data
...
def nodes_iterator_ngvs(values, pma_ips):
    ...
    for key, value in values.items():
        ...
        row_data['Hostname'] = IP
        status = update_status(list(value.values())[0]['tape.inp'])
        row_data['Backup status(Backup on  Archive Server)'] = status
        status1 = update_status(list(value.values())[0]['cassendra.inp'])
        row_data['Cassandra status(Backup on Disk)'] = status1
        status1 = update_status(list(value.values())[0]['zoo.inp'])
        row_data['Zookeeper  status(Backup on Disk)'] = status1
        final_data.append(row_data)
    return final_data
