...
#my $opco_name = 'ghana'; chomp($opco_name);
my $opco_name = $ARGV[0]; chomp($opco_name);
my $path = $FindBin::Bin;
$path =~ s/\/bin//ig;
...
my $to = ''; my $cc = '';
if (defined($ARGV[1])) { $to = $ARGV[1]; }
if (defined($ARGV[2])) { $cc = $ARGV[2]; }
...
sub tape{
	...
	if ($data =~ /INFO\:root\:Filesystem\s+backup\s+ended\s+at\s+$date1/ig) {
        ...
        elsif ($data =~ /INFO.*?root.*?Filesystem.*?backup.*?ended.*?at.*?$date1/ig) {
                $status = 'Success';
        }
	else {
		$status = 'Fail';
				...
				foreach my $inp_name (keys %$inp_hash) {	
					...
					foreach my $col_name (@$temp_ref) {
						...
						if ($inp_name eq $col_name) {
							...
							#$worksheet->write($row_num, ++$col_num, $fail_reason, $format6);
							if ($status eq $issue1) {
							...
							#}
                                        ...
                                        #$fail_flag = $fail_flag+2 if ($fail_flag>3);
...
sub set_sheet_headers{
	...
	foreach my $inp_name (@$header_ref) {
        ...
        #if ($tape_flag == 1) {
         #   $col_num++;
          #  $worksheet->write($row_num, $col_num, 'TAPE', $format4);
        #}
	$col_num++;
