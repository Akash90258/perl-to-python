...
def nodes_iterator_sdp(values, pma_ips):
    ...
    for key, value in values.items():
        ...
        if int(IP1.split('.')[-1]) > int(IP2.split('.')[-1]):
            ...
            res1 = list(value[IP1].values())[0]
            res  = list(value[IP2].values())[0]
            status1 = update_status(res1)
            status = update_status(res)
            row_data['Remark Node B'] = res1.split('^^')[-1]
            row_data['Remark Node A'] = res.split('^^')[-1]


            # if 'No Tape in Drive' in list(value[IP2].values())[0]:
            #     row_data['Remark Node A'] = "No Tape in Drive"
            # if 'No Tape in Drive' in list(value[IP1].values())[0]:
            #     row_data['Remark Node B'] = "No Tape in Drive"
        else:
            row_data['Node A-IP'] = IP1
            row_data['Node B-IP'] = IP2
            res1 = list(value[IP2].values())[0]
            res  = list(value[IP1].values())[0]
            status1 = update_status(res1)
            status = update_status(res)
            row_data['Remark Node B'] = res1.split('^^')[-1]
            row_data['Remark Node A'] = res.split('^^')[-1]
            # if 'No Tape in Drive' in res1:
                # row_data['Remark Node A'] = "No Tape in Drive"
            # if 'No Tape in Drive' in res:
                # row_data['Remark Node B'] = "No Tape in Drive"
        row_data['Backup Stauts Node-A'] = status
        row_data['Backup Stauts Node-B'] = status1
...
def Send_Email_SMTP(Attachment_Full_Path, flag):
    subject = 'New MTN Backup Tracker for Ghana'
    text = 'New MTN Backup Tracker for Ghana'
