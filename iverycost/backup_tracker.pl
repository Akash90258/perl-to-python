...
my $opco_name = 'ivorycoast'; chomp($opco_name);
...
my $pre_day1 = `date '+%d' -d '-1 day'`; chomp($pre_day1);
my $pre_day2 = trim(`date '+%e' -d '-1 day'`); chomp($pre_day2);
...
my $to = 'aditya.c.sharma@ericsson.com';
my $cc = '';

my $pre_day_hash = pre_day_hash();
#print Dumper \$pre_day_hash; die;
my $users_details = set_users_conf();
#print Dumper \$users_details; die;
read_opco_dir();
#print Dumper \$parsed_hash; die;
#print Dumper \$inp_dir_hash; die;
success_fail_count();
		...
		{	
			...
			foreach my $inp_name (@inp_files) {
				...
				if ($user_l eq 'sdp') {	
					...
					if ($inp_name =~ /tape.inp/ig || $inp_name =~ /nfs.inp/ig) {
						$status = tape($file_data, $curr_date, $curr_date5, $inp_dir_path);
						$parsed_hash->{$user}->{$host}->{$ip}->{$inp_name} = "$status^^$fail_reason";
					}
				}

				##----------------- VS geo-redundancy, nfs and cassendra Check -----------------##
				...
				if ($user_l eq 'vs') {
					...
					if ($inp_name =~ /tape.inp/ig || $inp_name =~ /nfs.inp/ig) {
						if (defined($pre_day_hash->{$user_l}->{lc($host)}->{$ip}->{lc($inp_name)})) {
							$status = tape($file_data, $pre_date4, $pre_date3);
						} else {
			...
			foreach my $date_val (@cassendra_arr) {
				if (defined($pre_day_hash->{$user_l}->{lc($host)}->{$ip}->{'cassendra.inp'})) {
					if ($date_val =~ /^$month\s+$pre_day1/ig || $date_val =~ /^$month\s+$pre_day2/ig) {
						$flag = 1;
					}
				} else {
			...
			if ($match == 1 && $cassandra_flag == 0) {
				$parsed_hash->{$user}->{$host}->{$ip}->{'cassendra.inp'} = '';		
			}
...
sub tape{
	...
	if ($data =~ /INFO\:root\:Filesystem\s+backup\s+ended\s+at\s+$date1/ig) {
		...
		$status = 'Fail';
		my $tape_data1 = ''; my $tape_data2 = '';
                $tape_data1 = readFile("$inp_dir_path/tape2.inp") if (-e "$inp_dir_path/tape2.inp");
                $tape_data2 = readFile("$inp_dir_path/tape1.inp") if (-e "$inp_dir_path/tape1.inp");
                my @tape_data1 = split('\n',$tape_data1);
...
sub dbn{
	...
	my $date = shift;
	my $status = '';
	if ($data =~ /ScheduledBackup\_$date/ig) {
		$status = 'Success';
	} else {
		$status = 'Fail';
	}
...
sub zoo{
	...
	my $date = shift;
	my $status = '';
	if ($data =~ /backup\_zk\_$date/ig || $data =~ /TCZookeeperbackup\_$date/ig || $data =~ /YoZookeeperbackup\_$date/ig) {
		$status = 'Success';
	} else {
		$status = 'Fail';
	}
		...
		foreach my $host (keys %$host_hash) {	
			...
			foreach my $ip (keys %$ip_hash) {
				...
				foreach my $inp_name (keys %$inp_hash) {
					...
					if ($parsed_hash->{$user}->{$host}->{$ip}->{$inp_name} eq '') {
						$success_count_hash->{$user}->{$inp_name}->{total} -= 1;
					}
				}
			}
...
sub pre_day_hash{
	my $hash = {};
	$hash->{'vs'}->{'svrvstc01b'}->{'10.102.193.24'}->{'tape.inp'} = 1;
	$hash->{'vs'}->{'svrvstc01c'}->{'10.102.193.25'}->{'tape.inp'} = 1;
	$hash->{'vs'}->{'svrvstc01a'}->{'10.102.193.23'}->{'tape.inp'} = 1;
	$hash->{'vs'}->{'svrvsyo01b'}->{'10.102.65.21'}->{'cassendra.inp'} = 1;
	$hash->{'vs'}->{'svrvsyo01a'}->{'10.102.65.20'}->{'cassendra.inp'} = 1;
	$hash->{'vs'}->{'svrvsyo01c'}->{'10.102.65.22'}->{'cassendra.inp'} = 1;
	return $hash;
}
