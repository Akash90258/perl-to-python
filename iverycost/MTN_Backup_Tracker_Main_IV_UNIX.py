...
log_file_path = "/home/guineabisau/PM-Automation/PythonBT/logs/logs_{}.log".format(str(date.today()))
logging.basicConfig(
    level=logging.INFO,
...
try:
    ...
    for key, values in parsed_hash.items():
        ...
        if 'SDP' == opco.upper():
            ...
            template_df_map[sheet_name] = template_df_map[sheet_name].reset_index()#.duplicated(keep='first')
            print(template_df_map[sheet_name].columns )
            # exit()
            for row in final_data:
                # template_df_map[sheet_name].dropna(subset=['Node Name'], inplace=True)
                # template_df_map[sheet_name] = template_df_map[sheet_name].concat(row)
                
                # template_df_map[sheet_name].concat([template_df_map[sheet_name],row]).drop_duplicates(keep=False)
                template_df_map[sheet_name] = template_df_map[sheet_name].loc[~template_df_map[sheet_name].index.duplicated(keep='first')]
                template_df_map[sheet_name] = template_df_map[sheet_name].loc[:,~template_df_map[sheet_name].columns.duplicated()]
                row = pd.DataFrame.from_records(row)
                row = row.to_frame('count')
                template_df_map[sheet_name] = template_df_map[sheet_name].append(row, ignore_index=True, verify_integrity=False).drop_duplicates().sort_values()
            # template_df_map = template_df_map.sort_values('Node Name')
            print(template_df_map[sheet_name])
            exit()
...
Config_File = glob.glob("/home/ivorycoast/PM-Automation/MTN/CONFIG/CONFIG_PYTHON_UNIX.xlsx")
